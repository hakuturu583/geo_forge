...
def _train_gaussians_on_sweeps(
    *,
    ...
    tile_size = 16
    before_dir = output_dir / "before" / f"{merge_idx:04d}"
    ...
    for step in range(int(config.steps_per_merge_idx)):
        ...
        if config.loss_weights.edge_aware.weight > 0.0:
            loss = loss + float(
                config.loss_weights.edge_aware.weight
    ...
    after_dir = output_dir / "after" / f"{merge_idx:04d}"
