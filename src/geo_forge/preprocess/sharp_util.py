...
def _load_sharp_gaussians_world(meta: dict[str, object]) -> Gaussians3D | None:
    ...
    # SHARP gaussians are unprojected in the (OpenGL-style) camera frame.
    # dataset c2w is already in the gs world frame (NuScenes->OpenGL + world->gs applied),
    # so apply it directly to lift gaussians to world coordinates.
    c2w = c2w_gs[:3, :]
