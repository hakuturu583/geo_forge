    ...
    def get_frames(
        ...
        **kwargs
        ...
        if cache_key not in self._scene_cache:
            self._scene_cache[cache_key] = self._load_scene_data(scene_name, camera_name)
        ...
        if self.verbose:
            print(f"Loaded {len(frames)} frames from scene: {scene_name}, camera: {camera_name}")
...
def example_usage():
    ...
    config = SAM3PreprocessorConfig(
        output_dir='./nuscenes_sam3_output',
        verbose=True
    )
    ...
    if scenes:
        ...
        results = preprocessor.process_with_adapter(
            adapter=adapter,
            identifier={'scene': scene_name, 'camera': 'CAM_FRONT'}
        )
